...
void DessinerPageHelp(AppState *etat, int zoneX) {
    float dpi = GetAppDPI();
    Vector2 origin = BeginPageContent(zoneX, &etat->heightHELP, &etat->scrollHELP);

    int margeX = (int)(30 * dpi);
    int largeurUtile = (GetScreenWidth() - zoneX) - (margeX * 2);
    ...
    DrawHelpCard(x + colW + gap, (int)y, colW, cardH, dpi, "SHIFT", "Mode LectureS");
    y += cardH + 35 * dpi;
    ...
    for(int i = 0; i < 4; i++) {
        DrawHelpCard(x, (int)y, largeurUtile, cardH, dpi, ondesT[i], ondesD[i]);
        y += cardH + (int)(5 * dpi);
    }
    y += 30 * dpi;
    ...
    for(int i = 0; i < 4; i++) {
        DrawHelpCard(x, (int)y, largeurUtile, cardH, dpi, adsrT[i], adsrD[i]);
        y += cardH + (int)(5 * dpi);
    }
    y += 35 * dpi;

    // --- 5. FOOTER ---
    Rectangle infoBox = { (float)x, y, (float)largeurUtile, (float)(60 * dpi) };
    ...
    EndPageContent(finalY, origin.y, &etat->heightHELP);
}