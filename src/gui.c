...
float TextToFloat(const char *text) { return strtof(text, NULL); }
...
static void DrawMenuButton(AppState *state, const char *label, PageApp page,
  ...
  Color background =
      active ? (Color){130, 180, 220, 255} : (Color){200, 200, 200, 255};
  Color border =
      active ? (Color){90, 140, 180, 255} : (Color){160, 160, 160, 255};

  DrawRectangle(x, y, width, height, background);
  DrawRectangleLines(x, y, width, height, border);

  if (GuiButton((Rectangle){(float)x, (float)y, (float)width, (float)height},
                label)) {
...
static void DrawSidebar(AppState *state) {
  ...
  // Sidebar background
  DrawRectangle(0, 0, sidebarWidth, GetScreenHeight(),
  ...
  DrawMenuButton(state, "#125#Oscillator" /*ICON_WAVE_SINUS*/, PAGE_OSCILLATOR,
                 x, y, w, h);
  y += h + s;
  DrawMenuButton(state, "#124#Envelope" /*ICON_WAVE*/, PAGE_ENVELOPE, x, y, w,
                 h);
  y += h + s;
  DrawMenuButton(state, "#122#Output" /*ICON_AUDIO*/, PAGE_OUTPUT, x, y, w, h);
  y += h + s;
  DrawMenuButton(state, "#193#Help" /*ICON_HELP*/, PAGE_HELP, x, y, w, h);
  y += h + s;
  DrawMenuButton(state, "#150#Piano", PAGE_PIANO, x, y, w, h);
  y += h + s;
  ...
  Rectangle btn = {(float)(x + 10 * dpi), (float)y, (float)(w - 20 * dpi),
                   (float)(45 * dpi)};

  // Color according to state
  Color c = state->audioActive ? (Color){70, 180, 110, 255}
                               : (Color){200, 70, 70, 255};
  ...
  DrawText(txt, (int)(btn.x + btn.width / 2 - tw / 2),
           (int)(btn.y + btn.height / 2 - fs / 2), fs, RAYWHITE);
  ...
  Rectangle btnMode = {(float)(x + 10 * dpi), (float)y, (float)(w - 20 * dpi),
                       (float)(45 * dpi)};
  ...
  const char *txtMode =
      (state->playbackMode == MODE_CONTINUOUS) ? "CONTINU" : "ENVELOPPE";
...
Vector2 BeginPageContent(int zoneX, float *contentHeight,
                         Vector2 *scrollState) {
  float dpi = GetAppDPI();
  Rectangle panelRec = {(float)zoneX, 0.0f, (float)(GetScreenWidth() - zoneX),
                        (float)GetScreenHeight()};

  float height = *contentHeight;
  if (height < panelRec.height)
    height = panelRec.height;

  Rectangle contentRec = {0.0f, 0.0f, panelRec.width - 20 * dpi, height};
...
void DrawAppInterface(AppState *state) {
  ...
  DrawRectangle(zoneX, 0, GetScreenWidth() - zoneX, GetScreenHeight(),
