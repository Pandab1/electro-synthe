...
static void DrawADSRVisualization(AppState *state, Rectangle zone) {
  ...
  // Normalize relative to maximums: Attack(2s) + Decay(2s) + Sustain(fixed) +
  // Release(3s)
  float totalTime =
      state->adsr.attack + state->adsr.decay + 1.0f + state->adsr.release;

  // Key points (X, Y)
  Vector2 pStart = {zone.x + margin, zone.y + zone.height - margin};

  // Attack
  Vector2 pAttack = {pStart.x + (state->adsr.attack / totalTime) * wUseful,
                     zone.y + margin};

  // Decay
  Vector2 pDecay = {pAttack.x + (state->adsr.decay / totalTime) * wUseful,
                    pAttack.y + (1.0f - state->adsr.sustain) * hUseful};

  // Sustain (give it a fixed width for visibility)
  Vector2 pSustainEnd = {pDecay.x + (1.0f / totalTime) * wUseful, pDecay.y};

  // Release
  Vector2 pEnd = {pSustainEnd.x + (state->adsr.release / totalTime) * wUseful,
                  pStart.y};
  ...
  int fontSize = 12;
  DrawText("A", (int)pStart.x, (int)(zone.y + zone.height - 15), fontSize,
           GRAY);
  DrawText("D", (int)pAttack.x, (int)(zone.y + zone.height - 15), fontSize,
           GRAY);
  DrawText("S", (int)pDecay.x, (int)(zone.y + zone.height - 15), fontSize,
           GRAY);
  DrawText("R", (int)pSustainEnd.x, (int)(zone.y + zone.height - 15), fontSize,
           GRAY);
}
...
void DrawEnvelopePage(AppState *state, int zoneX) {
  ...
  Vector2 origin =
      BeginPageContent(zoneX, &state->heightEnvelope, &state->scrollEnvelope);
  ...
  DrawText("Attack: Temps pour atteindre le volume max", x + 10, (int)(y + 30),
           (int)(11 * dpi), GRAY);
  DrawText("Decay: Temps pour descendre au niveau de sustain", x + 10,
           (int)(y + 50), (int)(11 * dpi), GRAY);
  DrawText("Sustain: Niveau maintenu pendant la note", x + 10, (int)(y + 70),
           (int)(11 * dpi), GRAY);
  DrawText("Release: Temps pour revenir au silence", x + 10, (int)(y + 90),
           (int)(11 * dpi), GRAY);
  ...
  DrawSliderWithButtons(rAttack, "TEMPS D'ATTAQUE", &state->adsr.attack, 0.0f,
                        2.0f, 0.05f, "%.2fs");
  ...
  DrawSliderWithButtons(rDecay, "TEMPS DE DECAY", &state->adsr.decay, 0.0f,
                        2.0f, 0.05f, "%.2fs");
  y += hSliderRec + spacing;
  ...
  DrawSliderWithButtons(rSustain, "NIVEAU DE SUSTAIN", &state->adsr.sustain,
                        0.0f, 1.0f, 0.05f, "%.2f");
  y += hSliderRec + spacing;
  ...
  DrawSliderWithButtons(rRelease, "TEMPS DE RELEASE", &state->adsr.release,
                        0.0f, 3.0f, 0.05f, "%.2fs");
  y += hSliderRec + spacing;

  // --- SECTION 3 : VISUALIZATION ---
  DrawText("VISUALISATION ADSR", x, (int)(y + 5), (int)(12 * dpi), DARKGRAY);
  Rectangle visualizationZone = {(float)x, y + 25 * dpi, (float)w,
                                 (float)(120 * dpi)};
  DrawADSRVisualization(state, visualizationZone);

  float finalY =
      visualizationZone.y + visualizationZone.height + 25 * dpi; // padding
