...
float adsr_process(ADSR *e) {
  ...
  case ENV_ATTACK:
    target = 1.01f; // Target slightly above 1.0 to ensure we actually hit 1.0
                    // quickly (Logarithmic approach never quite reaches target)
    rate = calculate_coef(e->attack) *
           3.0f; // Multiplier makes curve convex/punchy
  ...
  case ENV_DECAY:
    ...
    // If we are close enough to sustain level, switch state (optional, can just
    // stay in decay effectively)
  ...
  case ENV_SUSTAIN:
    ...
    e->value +=
        (e->sustain - e->value) * calculate_coef(0.05f); // Smooth transition
    break;
