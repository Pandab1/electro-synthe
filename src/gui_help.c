...
static void DrawHelpCard(int x, int y, int w, int h, float dpi, const char *key,
                         const char *desc) {
  ...
  Rectangle keyRect = {(float)x + keyPadding, (float)y + keyPadding,
                       (float)keyWidth, (float)h - (keyPadding * 2)};
...
static void DrawHelpDetailCard(int x, int y, int w, int h, float dpi,
                               const char *title, const char *desc) {
  Rectangle r = {(float)x, (float)y, (float)w, (float)h};
  DrawRectangleRec(r, (Color){235, 235, 235, 255});
  DrawRectangleLinesEx(r, 2, (Color){160, 160, 160, 255});

  DrawText(title, x + (int)(12 * dpi), y + (int)(10 * dpi), (int)(14 * dpi),
           GRAY);
  DrawText(desc, x + (int)(12 * dpi), y + (int)(32 * dpi), (int)(12 * dpi),
           BLACK);
}
...
void DrawHelpPage(AppState *state, int zoneX) {
  ...
  Vector2 origin =
      BeginPageContent(zoneX, &state->heightHELP, &state->scrollHELP);
  ...
  char volKeys[32];
  snprintf(volKeys, sizeof(volKeys), "#%d# / #%d#", ICON_ARROW_UP_FILL,
  ...
  char arrowKeys[32];
  snprintf(arrowKeys, sizeof(arrowKeys), "#%d# / #%d#", ICON_ARROW_LEFT_FILL,
           ICON_ARROW_RIGHT_FILL);
  DrawHelpCard(x, (int)y, colW, cardH, dpi, arrowKeys, "Changer d'onde");
  DrawHelpCard(x + colW + gap, (int)y, colW, cardH, dpi, "SHIFT",
               "Mode playback");
  y += cardH + 35 * dpi;
  ...
  const char *ondesD[] = {"Onde pure sans harmoniques",
                          "Contient uniquement les harmoniques impaires.",
                          "Harmoniques impaires avec décroissance rapide.",
                          "Contient toutes les harmoniques."};

  int detailH = (int)(60 * dpi);

  for (int i = 0; i < 4; i++) {
    DrawHelpDetailCard(x, (int)y, usefulWidth, detailH, dpi, ondesT[i],
                       ondesD[i]);
    y += detailH + (int)(5 * dpi);
  }
  y += 30 * dpi;
  ...
  const char *adsrD[] = {"Durée pour passer du silence au volume maximum.",
                         "Durée pour descendre du pic au niveau de sustain.",
                         "Niveau maintenu tant que la note est jouée. ",
                         "Durée pour revenir au silence après le relâchement."};

  for (int i = 0; i < 4; i++) {
    DrawHelpDetailCard(x, (int)y, usefulWidth, detailH, dpi, adsrT[i],
                       adsrD[i]);
    y += detailH + (int)(5 * dpi);
  }
  y += 35 * dpi;
  ...
  DrawText("raylib - www.raylib.com & raygui - github.com/raysan5/raygui",
           (int)infoBox.x + 15, (int)infoBox.y + 22, (int)(11 * dpi), DARKGRAY);
